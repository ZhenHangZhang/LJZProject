...
class LJZNetworkManager: NSObject {
    
    static let shared:LJZNetworkManager = {
        let instance = LJZNetworkManager()
        return instance
    }()
    
    func getRequest(urlString: String, params : [String : Any], success : @escaping (_ response :AnyObject)->(), failture : @escaping (_ error : AnyObject)->()) {
...
//            .validate(contentType: ["text/html"])
            .responseJSON { (response) in/*这里使用了闭包*/
                ...
                if let value111 = vaule as? Array<Any> {
                    success(value111 as AnyObject)
                }
                case .failure(let error):
                    failture(error as AnyObject)
                    ZHZDLog("error:\(error)")
                }
        }
    }
